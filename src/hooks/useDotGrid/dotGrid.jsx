...
*/
export function DotGrid({ cols = 24, count, color = 'cream', pattern = 'wave', size, magnetic = false, className, style }) {
  const dotCount = count ?? cols * 8;
  const dots = useMemo(() => {
    const dotClasses = ['dot'];
    if (magnetic) {
      dotClasses.push('magnetic', 'magnetic--subtle');
    }
    return Array.from({ length: dotCount }, (_, i) => (
      <span key={i} className={dotClasses.join(' ')} />
    ));
  }, [dotCount, magnetic]);
