...
// Reads a CSS custom property from an element as a number.
// parseFloat strips units automatically — "320ms" → 320, "5px" → 5.
// Falls back to `fallback` if the property is missing or not numeric.
...
// Applies a single state class to the .cursor-custom wrapper; CSS descendant
// selectors drive styling on each child from there — one toggle, not two.
const makeCursorCls = (cls, wrapperRef) => ({
  add:    () => wrapperRef.current?.classList.add(cls),
  remove: () => wrapperRef.current?.classList.remove(cls),
});
...
export function useMagnetic(sectionRef) {
  ...
  useEffect(() => {
    ...
    const startLoop = () => {
      ...
      dot     = Object.assign(document.createElement("div"), { className: "custom-cursor--dot" });
      ringEl  = Object.assign(document.createElement("div"), { className: "custom-cursor--ring" });
      wrapper = Object.assign(document.createElement("div"), { className: "cursor-custom" });
      wrapper.append(dot, ringEl);
      dotRef.current     = dot;
      ...
      // Read CSS config once per loop start — avoids getComputedStyle every frame.
      // Change these values in the CSS, not here.
      const cursorTargetEl      = document.querySelector("[data-cursor-target]");
      const ringLerp            = cssVar(ringEl, "--ring-lerp",            0.12);
      const velocitySensitivity = cssVar(ringEl, "--velocity-sensitivity", 0.012);
      const velocityMaxScale    = cssVar(ringEl, "--velocity-max-scale",   0.45);
      const targetOffset        = cssVar(cursorTargetEl, "--offset",       5);
      ...
      const loop = (ts) => {
        ...
        lastTs.current = ts;
        const alpha = 1 - Math.pow(1 - ringLerp, dt / (1000 / 60));
        ...
        if (!ringEl.classList.contains("custom-cursor--ring--btn-primary") && !ringEl.classList.contains("custom-cursor--ring--btn-secondary")) {
          ringEl.style.left = `${ringPos.current.x}px`;
          ringEl.style.top  = `${ringPos.current.y}px`;
        }

        const { hovering, clicking, inZone } = state.current;
        if (!hovering && !clicking && inZone && !ringEl.classList.contains("custom-cursor--ring--btn-primary") && !ringEl.classList.contains("custom-cursor--ring--btn-secondary")) {
          ringEl.style.setProperty("--velocity-scale", (1 + clamp(speed * velocitySensitivity, 0, velocityMaxScale)).toFixed(3));
        }
        ...
        if (activeBtn && !activeBtn.classList.contains("btn--no-target")) {
          ...
          if (cursorTarget?.classList.contains("cursor-target--visible")) {
            ...
            cursorTarget.style.top    = `${r.top    - targetOffset}px`;
            cursorTarget.style.left   = `${r.left   - targetOffset}px`;
            cursorTarget.style.width  = `${r.width  + targetOffset * 2}px`;
            cursorTarget.style.height = `${r.height + targetOffset * 2}px`;
          }
        }
  ...
  useEffect(() => {
    ...
    // Read timing config from CSS once — change these in the stylesheet, not here.
    const idleDelay = cssVar(document.documentElement, "--cursor-idle-delay", 1200);
    ...
    const activeCls = makeCursorCls("cursor-custom--active",   wrapperRef);
    const idleCls   = makeCursorCls("cursor-custom--idle",     wrapperRef);
    const hoverCls  = makeCursorCls("cursor-custom--hovering", wrapperRef);
    const clickCls  = makeCursorCls("cursor-custom--clicking", wrapperRef);
    ...
    const onMagneticLeave = ({ currentTarget: el }) => {
      ...
      // Spring constants — tune via CSS custom properties on .magnetic:
      // --spring-stiffness: higher = faster oscillation
      // --spring-damping:   lower  = more bounces before settling
      const stiffness = cssVar(el, "--spring-stiffness", 220);
      const damping   = cssVar(el, "--spring-damping",   8);
      const mass      = 1;
    ...
    const getRingRect = (ringEl) => {
      const naturalSize = parseFloat(getComputedStyle(ringEl).width) || 32;
      const left   = parseFloat(ringEl.style.left)   || ringPos.current.x;
      const top    = parseFloat(ringEl.style.top)    || ringPos.current.y;
      const width  = parseFloat(ringEl.style.width)  || naturalSize;
      const height = parseFloat(ringEl.style.height) || naturalSize;
    ...
    const snapTargetToBtn = (btn) => {
      ...
      if (!target) return;
      const o = cssVar(target, "--offset", 5);
    ...
    const makeBtnHandlers = (ringClass, targetClass) => {
      const onEnter = ({ currentTarget: btn, clientX, clientY }) => {
        if (btn.classList.contains("btn--no-target")) return;
        ...
        runMorph({
          ...
          fromOpacity,
          toOpacity: 0,
          duration: cssVar(ringEl, "--morph-enter-duration", 320),
      ...
      const onLeave = ({ currentTarget: btn, clientX, clientY }) => {
        ...
        const getLiveCursorRect = () => {
          const size = cssVar(ringRef.current, "--ring-size", 32);
          return { left: mouse.current.x, top: mouse.current.y, width: size, height: size };
        };
        ...
        runMorph({
          ...
          fromOpacity: 0,
          toOpacity:   1,
          duration: cssVar(ringEl, "--morph-leave-duration", 460),
          easing:   easeInOutSine,
          originBias: { dx, dy },
          onComplete: () => {
    ...
    } = makeBtnHandlers("custom-cursor--ring--btn-primary", "cursor-target--btn-primary");

    const {
      onEnter: onBtnSecondaryEnter,
      onLeave: onBtnSecondaryLeave,
      onMove:  onBtnSecondaryMove,
    } = makeBtnHandlers("custom-cursor--ring--btn-secondary", "cursor-target--btn-secondary");
    ...
    const btnEls = zone.querySelectorAll(".btn-primary:not(.btn--no-target)");
    btnEls.forEach((el) => {
    ...
    const btnSecondaryEls = zone.querySelectorAll(".btn-secondary:not(.btn--no-target)");
    ...
    // Regular hover state for non-primary/secondary buttons and btn--no-target CTAs
    const allBtnEls = zone.querySelectorAll(".btn:not(.btn-primary):not(.btn-secondary), .btn-primary.btn--no-target, .btn-secondary.btn--no-target");
    ...
    const bindBtn = (el) => {
      if (el.classList.contains("btn--no-target")) {
        el.addEventListener("mouseenter", onBtnEnter);
        el.addEventListener("mouseleave", onBtnLeave);
