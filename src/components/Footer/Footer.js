...
function Footer() {
  ...
  return (
    <footer className='footer section-padding'>
          ...
          <nav className='footer__nav' aria-label='Footer navigation'>
            <ul className='footer__nav-links menu__nav-links' sa='right-long slow mirror'>
              {NAV_LINKS.map(({ to, label, animationDelay, exact }) => (
                <li key={to} className={`menu__nav-link nav-link__${label.toLowerCase()} ${isActive(to, { exact }) ? "--is-active" : ""}`.trim()} data-cursor="light">
      ...
      <button
        className={`btn btn-secondary scroll-to-top${showScrollTop ? ' scroll-to-top--visible' : ''}`}
        onClick={scrollToTop}
